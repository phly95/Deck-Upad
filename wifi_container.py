...
class ContainerNetwork:
    ...
    def run_diagnostics(self):
        ...
        print("2. Host <-> Container Link (Ping 192.168.50.1)...")
        try:
            if self.run_command("ping -c 2 -W 2 192.168.50.1", check=False, timeout=5):
...
dhcp-option=6,8.8.8.8"""
    ...
    def setup_client_mode_with_discovery(self, ssid, password, ctr_pid):
        ...
        self.run_command(f"ip link set {VETH_CTR} netns {ctr_pid}")
        self.run_command(f"{self.exec_cmd} 'ip link set {VETH_CTR} up'")
        self.run_command(f"{self.exec_cmd} 'ip addr add 10.13.13.1/24 dev {VETH_CTR}'")

        self.run_command(f"nmcli connection add type ethernet ifname {VETH_HOST} con-name {NM_CONN_NAME} ip4 10.13.13.2/24 gw4 10.13.13.1 connection.zone trusted")
        ...
        if upstream:
            ...
            self.run_command(f"iptables -I FORWARD 1 -i {VETH_HOST} -o {upstream} -j ACCEPT", check=False)
            self.run_command(f"iptables -I FORWARD 1 -i {upstream} -o {VETH_HOST} -m state --state RELATED,ESTABLISHED -j ACCEPT", check=False)
            self.run_command(f"iptables -t nat -I POSTROUTING 1 -o {upstream} -j MASQUERADE", check=False)
